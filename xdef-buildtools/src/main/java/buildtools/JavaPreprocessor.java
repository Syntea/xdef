 ...
 * <p>Preprocessor commands separates source blocks by special Java comments. Each preprocessor command must start with
 * the sequence "/*#" followed by the name of command and the whole command must be specified on a separated line.
 * The end of command must be either '*&#47;' or '*#/'. Each command block must start with the "/*#if" command and
 * it must be closed with the "/*#end*&#47;" command. </p>
 ...
 * </ul>
 * <p>
 * If command block of lines starts with 'if' command and ends with the 'end' command. Inside in the command block
 * there may be optionally specified several sections 'elseif' and optionally there may be specified the an "else" 
 *section (it must be the the last one section of the command block. Nested "if" command blocks are permitted.
 * </p>
 * <p>Preprocessor works so that it modifies the character sequences ending command lines from '*&#47;' to '*#/'
 * and/or from '*#/' to '*&#47;' according to result of evaluation of the boolean expression given by values of switches
 * specified from outside environment. This causes that some sections of Java source code will either as comments
 * or the others will be "uncommented".
 * </p>
 ...
 *                              or 'J1.5' is set
 * </code></pre>
 * <p> The <code>set</code> command enables to set the value of switches on. The names of switches in the list are
 * separated by commas. The command MUST be placed in the <code>if ... end</code> block and it is executed only
 * if the block is active.</p>
 * <p> The <code>unset</code> command enables to set the value of switches off. The names of switches in the list are
 * separated by commas. The command MUST be placed in the <code>if ... end</code> block and it is executed only
 * if the block is active.</p>
 ...
 * <p>where:</p>
 * <ul>
 * <li> -i input Input may be specification of the file with input source or the directory. If the parameter is not
 * directory, also wildcards ('*' or '?' may be used to specify group of files. If this parameter specified a directory
 * only files with the extension 'java' are processed. The parameter is obligatory.</li>
 * <li> -o outputDirectory: The directory where output files are stored. The parameter is optional. If it is not
 * specified the source file is overwritten.</li>
 * <li> -s switch[,switch..]: the comma separated list of switch names. Each switch name is composed from letters,
 * digits, '.' or '_'s. If switch should be set to false it can be either prefixed with '!' or not specified.
 * The parameter is optional.</li>
 * <li> -r dirTree input directory. The parameter is optional and forces to process all subdirectories of the directory
 * where process started.</li>
...
public class JavaPreprocessor {
    ...
    private int processFiles(final File[] files,
        final String outDir,
    ...
    private int processFile(final File fi, final String outDir, final boolean createDirectory) {
        ...
        try {
            ...
            try (Writer out = _charset == null ? new OutputStreamWriter(new FileOutputStream(fo)) 
    ...
    /** Process file. */
    private int processFile(final BufferedReader br) throws IOException {
        ...
        while ((command = parseLine()) != EOF) {
            ...
            switch (command) {
                ...
                case UNSET_COMMAND:
                    ...
                    break;
                default:
                    if (command != SRC_LINE) {//preprocessor command without "IF".						
            ...
            switch (command) {
                case ELSEIF_COMMAND: //elseif
                    ...
                    if (sectionWasGenerated) {
                        ignore = true; //A section was already generated.
                    } else if (!ignore) {
                ...
                case IF_COMMAND: //if
                    ...
                    continue;
                case SRC_LINE: // This should never be!
    ...
    /** Canonize command line (i.e. ends with "*#/" or with "*&#47;") and return true if status of section was changed.
    ...
    private boolean canonizeAndChageCommand(final boolean ignore) {
        ...
        if (ignore ? !_line.contains("*/") : _line.contains("*/")) { //no modification of section
            _sb.append(_line);
            return false;
        }
     ...
     * to <code>out</code>.
     * @param extract If the value of this argument is true the preprocessor will extract all preprocessor commands.
     * @param cutTrailingSpaces If the value of this argument is true the preprocessor will cut all whiteSpaces
     * at the end of lines.
    ...
    private static String proc(final String input,
        ...
        if ((f = new File(input)).isDirectory()) {
            lines += jp.processFiles(f.listFiles(), s, dirTree, false);
        } else {
            ...
            if (dirTree) {
                ...
                if (f.isDirectory()) {
                    ...
                    try {
                        ...
                        files = f.listFiles();
                        for (File file : files) {
                            if (file.isDirectory()) {//don't create first directory
                                lines += jp.processFiles(new File[]{file},s,true,false);
                            }
                        }
                    } catch (IOException ex) {
                        jp._err.flush();
        ...
        return jp._errors > 0 ? "[ERROR] detected " + jp._errors + " error" + (jp._errors > 1 ? "s" : "") + ".": null;
    }
