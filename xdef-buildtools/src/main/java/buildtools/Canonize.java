...
/** Canonize sources.
 * <p>1. Remove all spaces after the last non-blank character at the end of line and replace leading spaces by tabs.</p>
...
public class Canonize {
    ...
    /** Canonize sources. Remove all trailing white spaces on all lines and handle with leading spaces on all lines
     * according to argument <tt>tabs</tt>. Insert or update header or _tail information to sources according to value
     * of arguments <tt>_hdr</tt> and <tt>_tail</tt>. If the argument <tt>dirTree</tt> is true, do it with all
     * specified files in child directories.
     ...
     * @param hdr If <tt>true</tt> then leading standard copyright information is inserted before the first line of Java
     * source or it replaces the existing one. The template for the copyright information is taken from the file
     * <tt>hdrinfo.txt</tt>the root directory <tt>java</tt> (under which are projects). If the argument's value is
     * <tt>false</tt> then the top of source remains unchanged.
     * @param tail If <tt>true</tt> then log information is added after the last line of Java source or it replaces
     * the existing one. The template used for the log information is taken from the file
     * <tt>tailinfo.txt</tt> in the root directory <tt>java</tt> (under which are projects). If the value of this
     * argument is <tt>false</tt> then the end source remains unchanged.
     */
