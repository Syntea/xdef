...
public class XScriptMacroResolver extends StringParser {
	...
	private boolean isXScriptName(final StringParser p) {
		...
		while (StringParser.getXmlCharType(c = p.getCurrentChar(), _xmlVersion)
			...
			if (c == ':') { // we allow one colon inside the name
				...
				c = p.nextChar();
				if (p.getXmlCharType(_xmlVersion) != StringParser.XML_CHAR_NAME_START) {
					p.setIndex(p.getIndex() - 1); // must follow name, ignore ':'
					break;
				}
				sb.append(':');
			}
	 ...
	 */
	private String parseMacro(int nestingLevel, final StringParser p) throws SRuntimeException {
		...
		if (macro == null) {
			...
			if ((macro = _macros.get(macName)) == null) {
				macError(nestingLevel, XDEF.XDEF483,macName); //Macro '&{0}' doesn't exist
				return null;
			}
		}
		String[] params = null;
		...
		if (p.isChar('(')) { // parameters
			...
			while (isXScriptName(p)) {
				...
				if ((delimiter = p.isOneOfChars("'\"")) == NOCHAR) {
					macError(nestingLevel, XDEF.XDEF493); //String specification expected
					return null;
				}
				...
				for(;;) {
					if (!p.chkBufferIndex()) {
						macError(nestingLevel, XDEF.XDEF403); //Unclosed string specification
						return null;
					}
					char c;
				...
				if (index >= 0) {
					if (params == null) {
						...
						System.arraycopy(macro.getParamValues(), 0, params, 0, params.length);
					}
					s = sb.toString();
					level = nestingLevel;
	 ...
	 */
	private static void macError(final int level, final long id, final Object... mod) {
		if (level > 1) {
			return;
		}
		if (level == 1) {
			throw new SRuntimeException(XDEF.XDEF498); //Error in nested macro
		} else {
	...
	public final void expandMacros(final SBuffer sb) {
		setSourceBuffer(sb);
		int ndx;
		if ((ndx = sb.getString().lastIndexOf("${")) < 0 || ndx + 3 >= sb.getString().length()) {
			return; //no macro
		}
		setSourceBuffer(sb);
		...
		try {
			...
			} while ((ndx = getSourceBuffer().lastIndexOf("${")) >= 0 && ndx + 3 < getEndBufferIndex());
	...
	public static void expandMacros(final SBuffer sb,
		...
		final byte xmlVersion,
		final Map<String, XScriptMacro> macros) {
		XScriptMacroResolver p = new XScriptMacroResolver(actDefName, xmlVersion, macros, reporter);
		p.expandMacros(sb);
	}
}
