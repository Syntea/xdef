...
import org.xdef.util.conv.xsd2xd.utils.DOMUtils;
	...
	public static Element processElement(Element elementElement,
		...
		if (!elementFilled) {
			...
			if (elementElement.hasAttribute("type")) {
				...
				String prefix = DOMUtils.getQNamePrefix(type);
				//setting type namespace
				String typeNamespace = DOMUtils.getNSURI(prefix,elementElement);
				...
				if ((DOMUtils.firstElementChildNS(elementElement,
					Utils.NSURI_SCHEMA,
					Utils.COMPLEX_TYPE)) != null) {
					//nothing to do here
					//element has simple type as child
				} else if ((typeElement = DOMUtils.firstElementChildNS(
					elementElement,
					Utils.NSURI_SCHEMA,
...
//					schemaURLStack.peek().getPath());
		...
		if (Utils.isSchemaChild(attributeElement)) {
			...
			if (!refElementStack.isEmpty()) {
				...
				Element sType = DOMUtils.firstElementChildNS(attributeElement,
	...
	public static Element processGroup(Element groupElement,
		...
		if (groupElement.hasAttribute("ref")) {
			...
			Element objectGroup = DOMUtils.firstElementChildNS(
	...
	public static void processExtension(Element extensionElement,
		...
		String typeNamespace = DOMUtils.getNSURI(prefix, extensionElement);
		//type namespace is same as schema namespace
		if (Utils.NSURI_SCHEMA.equals(typeNamespace)) {
			//base type is schema anyType
			if ("anyType".equals(DOMUtils.getQNameLocalpart(base))) {
				...
				if (Utils.isRedefineSchemaChild(
					...
					decl =
						GlobalDeclaration.getGlobalDeclarationInRedefinedSchema(
							DOMUtils.getQNameLocalpart(base),
							Utils.COMPLEX_TYPE,
							schemaURLStack.peek(),
							redefine,
							schemaElements);
