...
public class GlobalDeclaration {
	...
	public static GlobalDeclaration getGlobalDeclaration(String fullName,
		...
		String refPrefix = DOMUtils.getQNamePrefix(fullName);
		String refLocalName = DOMUtils.getQNameLocalpart(fullName);
		...
		if (!"".equals(refPrefix)) {
			...
			String namespace = DOMUtils.getNSURI(refPrefix, schemaElement);
	...
	private static GlobalDeclaration getGlobalDeclarationInSchema(
		...
		String type,
		Map schemaElements) {
		NodeList globalDeclarations = DOMUtils.getChildElementsNS(
		...
		NodeList redefines = DOMUtils.getChildElementsNS(
			(Element) schemaElements.get(schemaURL),
			Utils.NSURI_SCHEMA,
			Utils.REDEFINE);
		//iterating redefines
		for (int i = 0; i < redefines.getLength(); i++) {
			...
			NodeList redefinedDeclarations = DOMUtils.getChildElementsNS(
