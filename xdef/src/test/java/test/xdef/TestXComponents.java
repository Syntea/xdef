...
"</xd:def>";
			...
			xml = "<X a='20201211010101333'>3<Y a='1'/><Y a='2'/>2021-12-30</X>";//millis == 333
			xc = parseXC(xp,"", xml , null, reporter);
			assertNoErrorwarningsAndClear(reporter);
			assertEq("", chkCompoinentSerializable(xc));
			assertEq(xml, xc.toXml());
			xml = "<X a='20201211010101000'>3<Y a='1'/><Y a='2'/></X>";//millis == 000
			xc = parseXC(xp,"", xml , null, reporter);
			assertNoErrorwarningsAndClear(reporter);
			assertEq("", chkCompoinentSerializable(xc));
			assertEq(xml, xc.toXml());
...
"</xd:def>";
			...
			xdef = // test union
"<xd:def xmlns:xd='"+_xdNS+"' root=\"A\">\n" +
"  <xd:declaration> type s union(%item=[byte(-1), int(1, 100)]); </xd:declaration>\n"+
"  <A a='s'>s;</A>\n"+
...
//////////////////////////////////
		...
		// generate XCDPool from sources used in next tests
...
"</s:H>\n";
			...
			xd.setXDContext(xml);
			xc = xd.xcreateXComponent(new javax.xml.namespace.QName("soap", "s:H"), null, null);
			assertEq("", chkCompoinentSerializable(xc));
			assertEq(xml, xc.toXml());
			...
			int century = new java.util.GregorianCalendar().get(java.util.Calendar.YEAR);
...
"</B></A>";
			...
			assertEq(1, XComponentUtil.get((XComponent) XComponentUtil.get((XComponent) XComponentUtil.get(
				(XComponent) XComponentUtil.get((XComponent) XComponentUtil.get(xc, "B"), "B_1"), "C"), "B"),
				"b"));
...
"</A>";
			...
			assertTrue(new SDatetime((java.util.Calendar) XComponentUtil.getx(
