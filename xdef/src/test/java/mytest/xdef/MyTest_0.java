...
////////////////////////////////////////////////////////////////////////////////
	...
	/** Crate and compile X-components form XDPool.
	 * @param tempSrcDir the directory where are generated Java sources.
	 * @param xp XDPool from which the X-components created.
	 */
	private static void genXComponent(final String tempSrcDir, final XDPool xp){
		try {
			File f = new File(tempSrcDir);
			if (f.exists()) {
				SUtils.deleteAll(f, true);
			}
			f.mkdirs();
			GenXComponent.genXComponent(xp,
				tempSrcDir, "UTF-8", false, true).checkAndThrowErrors();
			compileSources(tempSrcDir);
...
"</xd:def>\n"+
"\n"+
"<xd:def name='E' root='A' acceptLocals='X,Y'>\n"+ // acceptLocals from X,Y, however define local
"<xd:declaration scope='local'>\n"+
" String xx() {return 'E:xx';}\n"+       
" type x eq('Ex');\n"+       
" type y eq('Ey');\n"+       
" int xxx = 1;\n"+
"  void a() {out(xx() + xxx + yy() + yyy);}\n"+
...
" String xx() {return 'Z:xx';}\n"+       
" String yy() {return 'Z:yy';}\n"+       
" type x eq('Zx');\n"+       
" type y eq('Zy');\n"+
" int xxx = 4;\n"+
" int yyy = 2;\n"+
...
"</xd:collection>";
            ...
            assertEq("Z:xx4Z:yy2", strw.toString());
            xd = xp.createXDDocument("B");
            xd.setStdOut(strw = new StringWriter());
            xml = "<A a='Xx' b='Zy'/>";
            assertEq(xml, parse(xd, xml, reporter));
            assertNoErrors(reporter);
            assertEq("X:xx2Z:yy2", strw.toString());
            xd = xp.createXDDocument("C");
            xd.setStdOut(strw = new StringWriter());
            xml = "<A a='Zx' b='Yy'/>";
            assertEq(xml, parse(xd, xml, reporter));
            assertNoErrors(reporter);
            assertEq("Z:xx4Y:yy3", strw.toString());
            xd = xp.createXDDocument("D");
            xd.setStdOut(strw = new StringWriter());
            xml = "<A a='Xx' b='Yy'/>";
            assertEq(xml, parse(xd, xml, reporter));
            assertNoErrors(reporter);
            assertEq("X:xx2Y:yy3", strw.toString());
            xd = xp.createXDDocument("E");
            xd.setStdOut(strw = new StringWriter());
            xml = "<A a='Ex' b='Ey'/>";
            assertEq(xml, parse(xd, xml, reporter));
            assertNoErrors(reporter);
            assertEq("E:xx1Y:yy3", strw.toString());
		} catch (Exception ex) {fail(ex);}
if(true)return;
		try {
			xdef =
