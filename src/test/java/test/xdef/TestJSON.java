	...
	private boolean check(final String source) {
		...
		try {
			...
			if (source.charAt(0) == '<') {
				...
				Object j1 = JSONUtil.xmlToJson(el1);
				xdef = KXmlUtils.nodeToString(XJUtil.jsonToXDef(j1), true);
				XDPool xp = compile(xdef);
				Element el2 = JSONUtil.jsonToXml(j1);
				Object j2 = JSONUtil.xmlToJson(el2);
				if (!JSONUtil.jsonEqual(j1, j2)) {
					System.err.println("JSON objects are not equal:");
					System.err.println(JSONUtil.toJSONString(j1, true));
					System.err.println(JSONUtil.toJSONString(j2, true));
					result = false;
				}
...
"<js:array xmlns:js='http://www.syntea.cz/json/1.0'>H M</js:array>";
			...
			assertEq(xml, el);
			js = JSONUtil.xmlToJson(el);
			assertTrue(check(JSONUtil.toJSONString(js)));
			xml =
...
"<js:array xmlns:js='http://www.syntea.cz/json/1.0'>H M</js:array>";
			...
			xp = compile(xdef);
			el = parse(xp, "", JSONUtil.jsonToXml(js), reporter);
			assertNoErrors(reporter);
			assertEq(JSONUtil.jsonToXml(js), el);
