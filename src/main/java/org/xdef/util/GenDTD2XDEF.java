...
*/
	...
	private void prepareGen() {
		...
		_doc = KXmlUtils.newDocument(XDConstants.XDEF31_NS_URI,"xd:def",null);
		_xdef = _doc.getDocumentElement();
		_xdef.setAttributeNS(XMLConstants.XMLNS_ATTRIBUTE_NS_URI,
			"xmlns:xd", XDConstants.XDEF31_NS_URI);
	}
	...
	private void genModel(Element xd, ElemDecl elem) {
		...
		if (elem._any) {
			Element any = xd.getOwnerDocument().createElementNS(
				XDConstants.XDEF31_NS_URI, "xd:any");
			any.setAttributeNS(XDConstants.XDEF31_NS_URI,
			...
			if (elem._numText > 0) {
				model.setAttributeNS(XDConstants.XDEF31_NS_URI,
					"xd:text", "* string()");
			}
			char seqType;
			char seqOccurs;
	...
	private void genRefItem(Element p,
		...
		if (!"".equals(occurs) || !"".equals(ref)) {
			...
			x.setAttributeNS(XDConstants.XDEF31_NS_URI, "xd:script", s);
		}
		if ("".equals(ref)) {
			if (elem._childList != null && elem._childList.size() > 0) {
				if (elem._numText > 0) {
					x.setAttributeNS(XDConstants.XDEF31_NS_URI,
						"xd:text", "* string()");
				}
	...
	private void genSequence(final Element p,
		...
		if (seq._type == SeqItem.CHOICE) {
			x= p.getOwnerDocument().createElementNS(XDConstants.XDEF31_NS_URI,
				"xd:choice");
			if (seq._occurs != 0) {
