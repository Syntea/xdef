/**
	...
	XDPool contains	compiled code generated form of all X-definitions. To compile
	X-definitions use the {@link cz.syntea.xdef.XDFactory} - see static methods.
	To execute a X-definition you must first create an object
	{@link cz.syntea.xdef.XDDocument}. This object contains pointer to the root
	X-definition where processing will start and it contains also all instances
	of objects needed for processing (variables etc). This
	object can be used for validation processing and/or for construction of XML data.
...
<li>
	...
	validates and processes input data represented from the input XML document. The
	important property of this mode is the ability to process XML files of unlimited
	size. If it is used the option "forget" in X-definitions the size of input
	data may be many gigabytes and it is not limited by the size of internal
	memory of the Java virtual machine. The speed of processor is
	typically over 1 megabyte per second. The method which provides this mode
	is {@link cz.syntea.xdef.XDDocument#xparse(java.io.File,
	cz.syntea.xdef.sys.ReportWriter)}.</li>
<li>
	Similar way of processing is the mode, where the XML document is available to
	the processor in the form of object of the <tt><i>org.w3c.dom.Document</i></tt>
	...
	mode is	{@link cz.syntea.xdef.XDDocument#xparse(org.w3c.dom.Node,
	cz.syntea.xdef.sys.ReportWriter)}.</li>
<li>
	...
	according to given X-definition (see {@link cz.syntea.xdef.XDDocument#xcreate(String,
	...
	{@link cz.syntea.xdef.XDDocument#getXMDefinition()}<br/>
	{@link cz.syntea.xdef.proc.XXNode#getXMElement()}<br/>
	{@link cz.syntea.xdef.proc.XXData#getXMData()}<br/>
