...
//		}
	...
	private boolean simpleStatement(final String oname) {
		String name = oname;
		SPosition spos = getLastPosition();
		boolean wasVariable;
		switch (_sym) {
				...
				while (_sym == DOT_SYM) {
					...
					if (numPar >= 0) { // '(' => method
						...
						if(!wasVariable && !wasClassMethod) {
							...
							if(!wasClassMethod) {
								//Variable is defined but the method was not defined
								//in X-Skript
								//Unknown method: '&{0}'
								error(XDEF.XDEF443, cname);
							}
						}
					} else {
						error(XDEF.XDEF410, "("); //'&{0}' expected
	...
	final short getTypeCode(final String typeName) {
		short varType = CompileBase.getTypeId(typeName);
		if (varType < 0) {
			error(XDEF.XDEF412); //Type identifier expected
	...
	private short getVarTypeCode(final String typeName) {
		short varType = getTypeCode(typeName);
...
//					var.setInitialized(true);// default initialization (null...)
	 ...
	 * @param local true if local scope.
	 ...
	 * @param local if true then method is declared in the local declaration.
	 */
	final void compileExtMethod(final boolean local) {
		...
		if (nextSymbol() == IDENTIFIER_SYM) {
			...
			if ("XXElement".equals(s) ||
				"XXData".equals(s) ||
				"XXNode".equals(s)) {
				classPar = "cz.syntea.xdef.proc." + s;
			} else {
	...
	private Method getExtMethod(
		...
		if (classPar == null) {
			...
			for (int i = 0; i < numPar; i++) {
				...
				if (s == null) {//array
					pars[i] = cz.syntea.xdef.XDValue[].class;
				} else {
			...
			for (int i = 0; i < numPar; i++) {
				String s = params.get(i);
				if (s == null) {//array
					pars[i + 1] = cz.syntea.xdef.XDValue[].class;
				} else {
	 ...
	 * statement.
	 * @param local if true the declaration has local scope
	...
	final void compileDeclaration(final boolean local) {
		...
		while (_sym != NOCHAR) {
			...
			} else if (_sym == TYPE_SYM || _sym == UNIQUE_SET_SYM) {
				 // compile type or uniqueSet
				compileTypeOrUniqueset((byte) 'G', local);
				continue;
			}
	 ...
	 */
	final void compileTypeOrUniqueset(final byte varKind, final boolean local) {
