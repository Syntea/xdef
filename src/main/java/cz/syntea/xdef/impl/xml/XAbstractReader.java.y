...
public abstract class XAbstractReader extends Reader {
	...
	public final void releaseScanned() {
		if (_pos > 4000) { // do it only if it makes sense
			_bf.delete(0, _pos);
//			_filePos +=  _pos;
			_len = _bf.length();
			_pos = 0;
	...
	private int scanStringToChar(final char quote) {
		int pos = _pos;
		long line = _line;
	...
	public final SPosition getBufferPosition(int off) {
		return new SPosition(0, _line, _startLine, _filePos + off, _sysId);
	}
	...
	public final List<Object[]> getElementPositions(final String qName) {
		...
		String name;
		SPosition spos = getBufferPosition(1);
		if ("*".equals(qName)) {
			int i = -1;
...
//					releaseScanned();
			...
			item[1] = getBufferPosition(1); //position
			if ((scanned = scanLiteral()) < 0) {
				break;
			}
			item[2] = _pos-1 > scanned +1 //value
				? _bf.substring(scanned +1, _pos-1) : "";
			result.add(item);
System.out.println("ATTR: " + item[0] + "=" + item[2] + "; " + item[1]);
			if (!"*".equals(qName)) {
//				releaseScanned();
			}
		}
...
//	}
//
//	private boolean chkChar(final char c) {
//		return (_pos >= _len) ? false : c == _bf.charAt(_pos);
//	}
//	
