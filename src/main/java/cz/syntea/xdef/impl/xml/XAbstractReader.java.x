...
//	public final String getUnparsed() {return _bf.substring(_pos);}
	...
	public final SPosition getBufferPosition(int off) {
		return new SPosition(0, _line, _startLine, _filePos+_pos + off, _sysId);
	}
	...
	public final List<Object[]> getElementPositions(final String qName) {
		...
		String name;
		SPosition spos = getCorrectedPosition(1);
		if ("*".equals(qName)) {
			int i = -1;
